...
df_map = pd.DataFrame.from_dict(resp_map_json) # defining DF for map
clean_df_map = df_map.drop(['examine', 'members', 'lowalch', 'limit', 'icon', 'value'], axis=1).sort_values(by=['id'], ascending=True).reset_index() #reformating
...
no_nan = df_merge.dropna() # dropped all NaN from DF
